...
class RRDBNet(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, num_filters=48, num_blocks=23, gc=32):
        super(RRDBNet, self).__init__()
        self.conv_first = nn.Conv2d(in_nc, num_filters, 3, 1, 1, bias=True)
        self.trunk = nn.Sequential(*[RRDB(num_filters, gc) for _ in range(num_blocks)])
        ...
        self.conv_last = nn.Conv2d(num_filters, out_nc, 3, 1, 1, bias=True)
